(function () {
  ...
  el('operation-files').innerHTML = data.files.map(row => `<tr>${row.map((value,index) => `<td class="${index === 5 && value === 'Bloqueado' ? 'access-blocked' : index === 5 && value === 'Parcial' ? 'access-partial' : ''}">${escape(value)}</td>`).join('')}</tr>`).join('');
  ...
  el('operation-evidence').innerHTML = data.evidence.map(([id,description,origin,condition,risk]) => `<article class="evidence-card"><b>EVIDÊNCIA ${escape(id)}</b><p>${escape(description)}</p><small>Origem: ${escape(origin)}</small><small>Condição: ${escape(condition)}</small><small>Risco: ${escape(risk)}</small></article>`).join('');
